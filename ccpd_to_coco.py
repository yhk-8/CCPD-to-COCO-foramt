...
def get_info(im_file):
    ...
    test = list(np.array(segmentation).flatten()) 
    segmentation_temp = [test]
    return [left, top, width, height], segmentation, segmentation_temp
...
def main():
    ...
    for im_file in im_files:
        ...
        bounding_box, segmentation, segmentation_temp = get_info(im_file)
        class_id = 1  # id 为数字形式，如 1,此时是list形式，后续需要转换 # 指定为1，因为只有”是车牌“这一类

        # 显示日志
        print(bounding_box, segmentation_temp)
        ...
        #                   args=(annotation_id, image_id, class_id, area,image.size, bounding_box,segmentation_temp),
